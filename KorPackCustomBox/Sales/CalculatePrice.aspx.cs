...
public partial class Sales_DirGroup : System.Web.UI.Page
{
    ...
    private void pageInitialize()
    {
        ...
        txtQtyRFQ.Text = "800";//  System.DateTime.Now.ToShortDateString();
        txtLengthRFQ.Text = "20";
        txtWidthRFQ.Text = "10";
        txtheightRFQ.Text = "5";
        txtTruckRFQ.Text = "2";
        txtOveLapRFQ.Text = "0";

        txtQtyPO.Text = "800";//  System.DateTime.Now.ToShortDateString();
        txtLengthPO.Text = "20";
        txtWidthPO.Text = "10";
        txtheightPO.Text = "5";
        txtTruckPO.Text = "2";
        ...
        ddlBroadGradeRFQ.DataSource = ds.Tables[1];
        ddlBroadGradeRFQ.DataTextField = "StrengthCId";
        ddlBroadGradeRFQ.DataValueField = "StrengthId";
        ddlBroadGradeRFQ.DataBind();

        ddlBroadGradePO.DataSource = ds.Tables[1];
        ddlBroadGradePO.DataTextField = "StrengthCId";
        ddlBroadGradePO.DataValueField = "StrengthId";
        ddlBroadGradePO.DataBind();
        string RoleID = "0";
        ...
        if (Session["UserID"] != null)
        {
            ...
            lblFinalPriceEachPO.Text = ds.Tables[3].Rows[0]["Final Each Price"].ToString();
            lblTotalPricePO.Text = ds.Tables[3].Rows[0]["Total For This Qty"].ToString();
        }
        ds.Dispose();
    ...
    protected void btnProcessPO_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            QuoteSentOrRFQorPlaceOrder(CBHAutoId,"1","");
    ...
    protected void btnProcessPORFQ_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string customer = dt.Rows[0]["CustomersCId"].ToString();
            sendemailoffice365ForRFQ(requestedByEmail, customer);
    ...
    protected void btnProcessOrder_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string requestedByEmail = dt.Rows[0]["Email"].ToString();
            string CBHAutoId = Session["CBHAutoId"].ToString();
            QuoteSentOrRFQorPlaceOrder(CBHAutoId, "3", txtPONumber.Text);
            dt = getOrderData(CBHAutoId);
            sendemailoffice365ForOrder(requestedByEmail, dt);
            sendemailoffice365ForOrderTo(requestedByEmail);

            //string myScript1 = "showInfo('Operation Completed. Email Sent Successfully.');";
            //ScriptManager.RegisterStartupScript(Page, this.GetType(), "ClientScript", myScript1, true);
            //myScript1 = "";
            Button8.Enabled = true;
            string myScript = "alert('Order Placed Successfully. Thank you.');";
    ...
    private void QuoteSentOrRFQorPlaceOrder(string CBHAutoId, string QuoteRFQPlaceOrder, string PONumber)
    {
        //1 = Quote Sent, 2=RFQ, 3=PlaceOrder
        string strSQL = "";
        strSQL = @"UPDATE CalculateBoxPrice_History SET QuoteSent="+ QuoteRFQPlaceOrder + ", PONumber='" + PONumber + "' WHERE CBHAutoId=" + CBHAutoId + @"";
        ExecuteSQL obAtten = new ExecuteSQL();
    ...
    private DataTable getOrderData(string orderNo)
    {
        ...
        strSQL = @"SELECT        CBHAutoId, Qty, Length, Width, Height, S.StyleCId, B.StrengthCId,
         ...
         convert(varchar(10), DATEADD(dd, 30, convert(datetime,insertdate)),101) ExpireDate, PONumber
    ...
    private DataTable getEmailID()
    {
        ...
        string strSQL = "";
        strSQL = @"SELECT U.Email, isnull(C.CustomersCId,'') CustomersCId
        FROM [dbo].[TB_PrmUserInfo] U 
		left join CB_Customers C on C.CustomersId= U.CustomerId 
    ...
    private void sendemailoffice365ForRFQ(string semail, string _customer)
    {
        ...
        msg.Bcc.Add(new System.Net.Mail.MailAddress("nnovy@korpack.com", "Nick Novy"));
        msg.Bcc.Add(new System.Net.Mail.MailAddress("faaruk@yahoo.com", "Korpack Orders"));

        //msg.Subject = "RFQ by " + Session["UserName"].ToString();
        msg.Subject = Session["UserName"].ToString() + " has requested for RFQ. Please reply as soon as possible.";
        msg.Body = emailbodyRFQ(_customer);
        msg.IsBodyHtml = true;
    ...
    private void sendemailoffice365ForOrder(string semail, DataTable _dt)
    {

        System.Net.Mail.MailMessage msg = new System.Net.Mail.MailMessage();
        msg.From = new System.Net.Mail.MailAddress("quotes@korpack.com", "KORPACK");
        msg.To.Add(new System.Net.Mail.MailAddress(semail, semail));
        msg.To.Add(new System.Net.Mail.MailAddress("orders@korpack.com", "Korpack Orders"));
        msg.Bcc.Add(new System.Net.Mail.MailAddress("nnovy@korpack.com", "Nick Novy"));
        msg.Bcc.Add(new System.Net.Mail.MailAddress("faaruk@yahoo.com", "Korpack Orders"));

        //msg.Subject = "RFQ by " + Session["UserName"].ToString();
        msg.Subject = Session["UserName"].ToString() + " has placed an order.";
        msg.Body = emailbodyOrder(_dt);
        msg.IsBodyHtml = true;

        System.Net.Mail.SmtpClient client = new System.Net.Mail.SmtpClient();
        client.UseDefaultCredentials = false;
        client.Credentials = new System.Net.NetworkCredential("quotes@korpack.com", "Dato0401");
        //client.Credentials = new System.Net.NetworkCredential("orders@korpack.com", "Calu0829");
        client.Port = 587; // You can use Port 25 if 587 is blocked (mine is!)
        client.Host = "smtp.office365.com";
        client.DeliveryMethod = System.Net.Mail.SmtpDeliveryMethod.Network;
        client.EnableSsl = true;
    ...
    private void sendemailoffice365ForOrderTo(string semail)
    {
        System.Net.Mail.MailMessage msg = new System.Net.Mail.MailMessage();
        msg.From = new System.Net.Mail.MailAddress("quotes@korpack.com", "KORPACK");
        msg.To.Add(new System.Net.Mail.MailAddress(semail, semail));
        ...
        msg.Subject = "Thank you from Korpack for placing your order! ";
        msg.Body = "Hello " + Session["UserName"].ToString() + ", <br/><br/> Thank you. We will get back to you soon.<br/><br/><br/> Best Regards,<br/>Team Korpack";
        msg.IsBodyHtml = true;
    ...
    private string emailbodyRFQ(string _customer)
    {
        string strStyle = ddlStyleRFQ.SelectedItem.Text;
        string strBoard = ddlBroadGradeRFQ.SelectedItem.Text;
        string strQty = txtQtyRFQ.Text;
        string strLength = txtLengthRFQ.Text;
        string strWidth = txtWidthRFQ.Text;
        string strHeight = txtheightRFQ.Text;
            ...
            <td>
                <table style='width: 700px'>
                    <tr>
                        ...
                        <td style='text-align: right; width: 350px'>
                            <div style='background-color: Navy; color: white; font-size: 20px; font-weight: 700; text-align: center'>RFQ</div>
                            <br />
                            Customer: " + _customer + @"<br />
                            RFQ Date: " + System.DateTime.Now.ToShortDateString() + @"<br />
                        </td>
                    </tr>
        ...
        <tr>
            <td style='border: solid 1px black'>
                <table style='width: 700px'>
                    ...
                    <tr>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + strQty + @"</td>                        
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + strLength + @"</td>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + strWidth + @"</td>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + strHeight + @"</td>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + strStyle + @"</td>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + strBoard + @"</td>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 87px;'>" + "" + @"</td>
                        <td style='font-size: 14px; font-weight: bold; text-align: center; width: 91px;'>" + "" + @"</td>
                    </tr>
                    <tr>
                        <td colspan='8'>&nbsp;</td>
                    </tr>
    ...
    private string emailbodyOrder(DataTable _dt)
    {
        string body = @"<table>
        <tr>
            <td>
                <table style='width: 700px'>
                    <tr>
                        ...
                        <td style='text-align: right; width: 350px'>
                            <div style='background-color: Navy; color: white; font-size: 20px; font-weight: 700; text-align: center'>Order</div>
                            <br />
                            Customer: " + _dt.Rows[0]["CustomersCId"].ToString() + @"<br />
                            Order Date: " + _dt.Rows[0]["QuoteDate"].ToString() + @"<br />
                        </td>
                    </tr>
        ...
        <tr>
            <td style='border: solid 1px black'>
                <table style='width: 700px'>
                    ...
                    <tr>
                        <td style='background-color: white; color: black; font-size: 16px; font-weight: 500; text-align: center; width: 233px'>" + _dt.Rows[0]["PONumber"].ToString() + @"</td>
                        <td style='background-color: white; color: black; font-size: 16px; font-weight: 500; text-align: center; width: 233px'>Net due in 75 days</td>
                        <td style='background-color: white; color: black; font-size: 16px; font-weight: 500; text-align: center; width: 234px'>Novy, Nick</td>
                    </tr>
                </table>
             ...
             <td style='border: solid 1px black'>
                <table style='width: 700px'>
                    ...
                    <tr>
                        <td style='text-align: left; width: 450px'>&nbsp;</td>
                        <td style='text-align: right; width: 350px;font-size: 16px; font-weight: bold;'>
                            RFQ Total: " + _dt.Rows[0]["TotalForThisQty"].ToString() + @"
                        </td>
                    </tr>
                    <tr>
                        <td colspan='2'>&nbsp;</td>
